...
?>
			...
			<div class="attr">
				<? 
				//Вывод свойств светильника
				foreach($arResult['PROPERTIES'] as $key=>$row){
					if(strstr($key,'_FIELD')){
						if(is_array($row['VALUE'])){
							$val_PROPERTIES = implode('<br> ',$row['VALUE']);
							if(!empty($val_PROPERTIES) || !empty($row['DEFAULT_VALUE'])){
								echo '<table><tr><td><b>'.$row['NAME'].':</b> </td><td>'.$val_PROPERTIES.'</td></tr></table>';
							}
							
						}
						else if(!empty($row['VALUE']) || !empty($row['DEFAULT_VALUE'])){
							echo '<p><b>'.$row['NAME'].':</b> '. (empty($row['VALUE']) ? $row['DEFAULT_VALUE'] : $row['VALUE']) .'</p>';
						}
					}
				}?>
</div>
