;(function (window)
{
	...
	BX.MessengerCommon.prototype.chatListPrepare = function(params)
	{
		...
		for (var i = 0; i < category.length; i++)
		{
			...
			if (groupElements[i].length <= 0 && (category[i].id == 'lines' || category[i].id == 'call' || category[i].id == 'extranet' || category[i].id == 'blocked' || category[i].id == 'bot'))
				continue;

			chatList.appendChild(BX.create("div", {props : { className: "bx-messenger-chatlist-group"}, children : [
				(category[i].id == 'lines' || category[i].id == 'call' || category[i].id == 'blocked' || category[i].id == 'bot')? null: BX.create("span", {attrs: {'data-type': category[i].id}, props : { title: category[i].title, className: "bx-messenger-chatlist-group-add"}}),
